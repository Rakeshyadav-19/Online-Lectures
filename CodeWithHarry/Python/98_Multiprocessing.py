...
# to use multiprocessing we need to create a process object which calls a start() method. The start() method runs the process and then to stop the execution, we use the join() method.
def my_func():
...
# Creating a pool of worker processes - create a pool of worker processes and then assign tasks to them as needed - queue is a data structure that allows data to be inserted at one end and removed from the other end, queue can be used to pass data between processes.
